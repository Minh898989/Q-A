...
export default function GroupPostPage({ groupId, basePath }) {
    basePath = basePath || `/forum/group/${groupId}/post`;
    ...
    const fetchPosts = useCallback(async () => {
        try {
            const res = await fetch(`https://api-phu-6.onrender.com/api/post/${groupId}`, {
                headers: {
    ...
    const handleFilter = useCallback(async (filterValues = {}) => {
        try {
            ...
            console.error('Lỗi lọc bài viết nhóm:', err);
        }
    }, [groupId]);
    ...
    }, [search, fetchPosts, handleFilter]);
    ...
    const handleCreatePost = async (formData) => {
        try {
            ...
            if (res.ok) {
                ...
                alert('Tạo bài viết nhóm thất bại: ' + (result.message || 'Lỗi không xác định'));
            }
        } catch (error) {
            console.error('Lỗi tạo bài viết nhóm:', error);
            alert('Tạo bài viết nhóm thất bại do lỗi mạng');
        }
    };
        ...
        <div className="min-h-screen bg-gray-100 font-sans relative">
            ...
            {showCreateForm && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
                    <PostForm onClose={() => setShowCreateForm(false)} onSubmit={handleCreatePost} />
                </div>
            )}
        </div>
    );
